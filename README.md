...
| `docs/comprobar-dominio.sh` | **El de fuera.** Pregunta lo que ve un desconocido: que las direcciones respondan sin redirección, que el canonical diga lo mismo que la dirección, que `.html` lleve a su versión sin extensión, que `http` lleve a `https`, que `www` lleve al dominio raíz y que el material de trabajo dé 404. Sirve antes y después de conectar el dominio. | `bash docs/comprobar-dominio.sh [host] [dominio canónico]` |
| `docs/configurar-dominio.sh` | **El de la conexión.** Deja el dominio sirviendo el sitio: los dos `CNAME` hacia el proyecto de Pages, SSL en *Full (strict)*, *Always Use HTTPS*, la ofuscación de correos **apagada** (Cloudflare la enciende sola y sin JavaScript el visitante ve «[email protected]» en el aviso de privacidad) y el 301 de `www` al dominio raíz. Idempotente, y **no borra reglas ajenas**: al reescribir el conjunto de redirecciones conserva las que no son suyas, como el enlace corto `/qr` del código QR. | `bash docs/configurar-dominio.sh camena.com.mx camena [--aplicar]` |
...
| `docs/qr.py` | **El del código QR.** Hace el código que lleva a CAMENA para redes y WhatsApp: 1080×1080, tinta sobre papel de la casa, zona de silencio de cuatro módulos y corrección de errores en Q para que aguante que la imagen se recomprima. Lleva al enlace corto **`camena.com.mx/qr`**, no a la portada: la regla que resuelve ese enlace vive en Cloudflare (Rules → Redirect Rules, fase `http_request_dynamic_redirect`) y hoy manda a la portada, así que cambiar de destino no obliga a rehacer el material que ya está en la calle. **Se verifica solo**: después de dibujarlo lo vuelve a abrir con OpenCV y, si no lee exactamente la dirección, borra el archivo y falla. | `python3 docs/qr.py` |
